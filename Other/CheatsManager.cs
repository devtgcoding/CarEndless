...
//             SkipToReward();
        ...
        if (Manager<UIManager>.Get().ActivePageName == "IngamePage")
        {
            ...
            {
                LevelRoot.Instance.BroadcastMessage("StartRoadWorks", GameObject.FindGameObjectWithTag("Player").rigidbody.transform.position.z);
                LevelRoot.Instance.BroadcastMessage("StartGuardRail", GameObject.FindGameObjectWithTag("Player").rigidbody.transform.position.z);
            }
