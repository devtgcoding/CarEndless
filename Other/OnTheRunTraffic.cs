...
public class OnTheRunTraffic : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (started && (newVehicleDistance == -1.0f || player.rigidbody.position.z >= newVehicleDistance))
        {
            float ratio = 1.0f - Mathf.Clamp01((player.rigidbody.position.z - startDistanceLane) / (endDistanceLane - startDistanceLane)),
            ...
            {
                spawnBonus = (player.rigidbody.position.z >= newBonusDistance && newBonusDistance != -1.0f && !player.TurboOn);
                GameObject instance = (GameObject)Instantiate((spawnBonus ? bonusPrefab : trafficVehicles[UnityEngine.Random.Range(0, trafficVehicles.Count)]), new Vector3(player.rigidbody.position.x, 0.0f, player.rigidbody.position.z + 200.0f), Quaternion.identity);
            ...
            newVehicleDistance = player.rigidbody.position.z + UnityEngine.Random.Range(minDistance, minDistance);
            ...
            if (newBonusDistance == -1.0f || spawnBonus)
            {
                ...
                newBonusDistance = player.rigidbody.position.z + UnityEngine.Random.Range(minDistance, minDistance);
            }
        }

        float playerZ = player.rigidbody.position.z;
        for (int i = opponents.Count - 1; i >= 0; --i)
        {
            GameObject opponent = opponents[i];
            if (opponent != null)
            {
                if (playerZ - opponent.rigidbody.position.z > 20.0f || opponent.rigidbody.position.y > 10.0f || (player.TurboOn && opponent.collider.isTrigger))
                {
