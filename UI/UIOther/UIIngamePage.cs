...
//			Debug.LogWarning("devo resumare musica");
    ...
    public void SetIndicatorActive(int id, bool active, bool withAnim)
    {
        ...
        if (active && withAnim)
        {
            turboIndicators[id].active.animation.Rewind();
            turboIndicators[id].active.animation.Play();
        }
        else if (active)
        {
            turboIndicators[id].active.animation.Play();
            turboIndicators[id].active.animation["TurboBolt@"].time = turboIndicators[id].active.animation["TurboBolt@"].length;
            turboIndicators[id].active.animation.Sample();
            turboIndicators[id].active.animation.Stop();
        }
    }
    #endregion
